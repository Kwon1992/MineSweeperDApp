...
// Connect Metamask - DApp(MineSweeper) 
...
initWeb3 = async () => {
  ...
  this.gameContract = new this.web3.eth.Contract(gameContractABI, gameContractAddress); // Contract 연결!!
...
function render() {
  ...
  if (hitBomb) { // 폭탄을 건드렸다면...
    ...
    // $.ajax({
    //   success: function sendTransaction(a) {
    //   }
    // });
    this.gameContract.methods.endGame(SHA256(bombCoordList),false).send({from:this.account});
    popUP();
...
function wrapWindowByMask(){
	...
	$('#blinder').css({'width':maskWidth,'height':maskHeight});  
	
	//애니메이션 효과 - 일단 1초동안 까맣게 됐다가 80% 불투명도로 간다.
	//$('#mask').fadeIn(1000);      
	$('#blinder').fadeTo("slow",0.6);    
}

jQuery(function($) {
	var loading = $('<img alt="loading" id="loading_bar" src="./images/loading_bar.gif">')
